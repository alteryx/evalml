...
def explain(
    ...
    if min_importance_threshold > 1 or min_importance_threshold < 0:
        raise ValueError(f'The minimum importance threshold must be a percentage value between 0 and 1, not {min_importance_threshold}')

    linear_importance = False
    if pipeline.estimator.model_family == ModelFamily.LINEAR_MODEL and importance_method == "feature":
...
def get_influential_features(imp_df, max_features=5, min_importance_threshold=0.05, linear_importance=False):
    ...
    return (
      list(imp_features[imp_features["importance"]>=heavy_importance_threshold]["feature"]),
      list(imp_features[imp_features["importance"].between(min_importance_threshold, heavy_importance_threshold)]["feature"]),
