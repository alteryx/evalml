...
def graph_partial_dependence(
    ...
    if isinstance(pipeline, evalml.pipelines.MulticlassClassificationPipeline):
        ...
        class_labels_strs = {class_label: str(class_label) for class_label in class_labels}
        # Don't specify share_xaxis and share_yaxis so that we get tickmarks in each subplot
        fig = _subplots.make_subplots(rows=rows, cols=cols, subplot_titles=class_labels_strs)
        for i, label in enumerate(class_labels):
            ...
            if mode == "two-way":
                ...
                if is_categorical:
                    ...
                    trace = _go.Scatter(x=x, y=y, line=dict(width=3), name=class_labels_strs[label])
        ...
        if mode == "two-way":
            ...
            xrange = (
                _calculate_axis_range(part_dep["feature_values"])
