...
def train_pipeline(pipeline, X, y, automl_config, schema=True):
    ...
    if (
        ...
        test_size_ = pipeline.forecast_horizon/len(X) if is_time_series(automl_config.problem_type) else 0.2
        X, X_threshold_tuning, y, y_threshold_tuning = split_data(
            X, y, pipeline.problem_type, test_size=test_size_, random_seed=pipeline.random_seed
        )
