...
class SMOTESampler(BaseSampler):
    ...
    def __init__(self, sampling_ratio=0.25, sampling_ratio_dict={}, k_neighbors=5, n_jobs=-1, random_seed=0, **kwargs):
        ...
        Arguments:
            ...
            sampling_ratio_dict (dict): Dictionary which has keys corresponding to each class, and the values are the number of samples we want to oversample to for each class key.
                If this value is provided, it will be used. Otherwise, we opt to use sampling_ratio. Defaults to empty.
    ...
    def fit(self, X, y):
        ...
        if len(self.parameters['sampling_ratio_dict']):
...
class SMOTENCSampler(BaseSampler):
    ...
    def __init__(self, categorical_features=[], sampling_ratio=0.25, sampling_ratio_dict={}, k_neighbors=5, n_jobs=-1, random_seed=0, **kwargs):
