...
class DropByType(ColumnSelector):
    """Drops columns by specified Woodwork logical type or semantic tag in input data.
    ...
    name = "Drop Columns By Type Transformer"
    hyperparameter_ranges = {}
    """{}"""
    needs_fitting = False
    ...
    def _modify_columns(self, cols, X, y=None):
        typed_columns = X.ww.select(cols).columns
        return X.ww.drop(typed_columns)
