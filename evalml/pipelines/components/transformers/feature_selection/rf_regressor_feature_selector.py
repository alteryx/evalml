...
class RFRegressorSelectFromModel(FeatureSelector):
    ...
    def __init__(self, number_features=None, n_estimators=10, max_depth=None,
                 percent_features=0.5, threshold=-np.inf, n_jobs=-1, random_state=0, parameters=None):
        max_features = None
        if parameters is not None and 'number_features' in parameters:
            number_features = parameters['number_features']
        if number_features:
