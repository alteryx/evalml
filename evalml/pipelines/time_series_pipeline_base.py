...
class TimeSeriesPipelineBase(PipelineBase, metaclass=PipelineBaseMeta):
    ...
    def predict(self, X, objective=None, X_train=None, y_train=None):
        ...
        Raises:
            ValueError: If y_train has no value or if final component is not an Estimator.
        ...
        try:
            ...
            raise ValueError(
                "Make sure to have a value that isn't None for y_train when calling time series' predict."
            )
        if self.estimator is None:
            raise ValueError(
