    ...
    def validate(self, X, y=None):
        ...
        Example:
            ...
            >>> assert null_check.validate(df) == {"errors": [],\
                                                   "warnings": [DataCheckWarning("Column 'lots_of_null' is 80.0% or more null", "HighlyNullDataCheck")]}
        """
        messages = {
            "warnings": [],
            "errors": []
        ...
        if self.pct_null_threshold == 0.0:
            ...
            messages["warnings"].extend([DataCheckWarning(warning_msg.format(col_name), self.name) for col_name in all_null_cols])
        else:
            ...
            messages["warnings"].extend([DataCheckWarning(warning_msg.format(col_name, self.pct_null_threshold * 100), self.name) for col_name in highly_null_cols])
        return messages
