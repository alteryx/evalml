...
class HighVarianceCVDataCheck(DataCheck):
    ...
    def validate(self, pipeline_name, cv_scores):
        ...
        Example:
            ...
            >>> assert check.validate("LogisticRegressionPipeline", cv_scores) == {"warnings": [DataCheckWarning("High coefficient of variation (cv >= 0.1) within cross validation scores. LogisticRegressionPipeline may not perform as estimated on unseen data.", "HighVarianceCVDataCheck")],\
                                                                                   "errors": []}
        """
        messages = {
            "warnings": [],
        ...
        if high_variance_cv:
            ...
            messages["warnings"].append(DataCheckWarning(warning_msg, self.name).to_dict())
        return messages
