...
def test_invalid_target_data_check_nan_error():
    ...
    assert invalid_targets_check.validate(X, y=pd.Series([np.nan, np.nan, np.nan])) == {
        "warnings": [],
        "errors": [DataCheckError("3 row(s) (100.0%) of target values are null", "InvalidTargetDataCheck")]
    }
...
def test_invalid_target_data_check_numeric_binary_classification_error():
    ...
    assert invalid_targets_check.validate(X, y=pd.Series([1, 5, 1, 5, 1, 1])) == {
        "warnings": [],
        "errors": [DataCheckError("Numerical binary classification target classes must be [0, 1], got [1, 5] instead", "InvalidTargetDataCheck")]
    }
    assert invalid_targets_check.validate(X, y=pd.Series([0, 5, np.nan, np.nan])) == {
        "warnings": [],
        "errors": [DataCheckError("2 row(s) (50.0%) of target values are null", "InvalidTargetDataCheck"),
    ...
    assert invalid_targets_check.validate(X, y=pd.Series([0, 1, 1, 0, 1, 2])) == {
        "warnings": [],
        "errors": [DataCheckError("Target does not have two unique values which is not supported for binary classification", "InvalidTargetDataCheck")]
    }
...
def test_invalid_target_data_check_invalid_data_types_error():
    ...
    assert invalid_targets_check.validate(X, y) == {
        "warnings": [],
        "errors": [DataCheckError("Target is unsupported {} type. Valid target types include: {}".format(y.dtype, ", ".join(valid_data_types)), "InvalidTargetDataCheck"),
...
def test_invalid_target_data_input_formats():
    ...
    assert messages == {
        "warnings": [],
        "errors": [DataCheckError("3 row(s) (75.0%) of target values are null", "InvalidTargetDataCheck"),
                   DataCheckError("Target does not have two unique values which is not supported for binary classification", "InvalidTargetDataCheck")]
    }
