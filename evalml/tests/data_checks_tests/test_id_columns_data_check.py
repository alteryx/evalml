...
def test_id_columns_warning():
    ...
    assert id_cols_check.validate(X) == DataCheckResults(warnings=[DataCheckWarning("Column 'Id' is 95.0% or more likely to be an ID column", "IDColumnsDataCheck"),
    ...
    X = pd.DataFrame.from_dict(X_dict)
    id_cols_check = IDColumnsDataCheck(id_threshold=1.0)
    assert id_cols_check.validate(X) == DataCheckResults(warnings=[DataCheckWarning("Column 'Id' is 100.0% or more likely to be an ID column", "IDColumnsDataCheck"),
...
def test_id_cols_data_check_input_formats():
    ...
    assert id_cols_check.validate(pd.DataFrame()) == DataCheckResults()

    #  test list
    assert id_cols_check.validate([1, 2, 3, 4, 5]) == DataCheckResults(warnings=[DataCheckWarning("Column '0' is 80.0% or more likely to be an ID column", "IDColumnsDataCheck")])

    #  test pd.Series
    assert id_cols_check.validate(pd.Series([1, 2, 3, 4, 5])) == DataCheckResults(warnings=[DataCheckWarning("Column '0' is 80.0% or more likely to be an ID column", "IDColumnsDataCheck")])

    #  test 2D list
    assert id_cols_check.validate([[0, 1], [1, 2], [2, 3], [3, 4], [4, 5]]) == DataCheckResults(warnings=[DataCheckWarning("Column '0' is 80.0% or more likely to be an ID column", "IDColumnsDataCheck"),
                                                                                                          DataCheckWarning("Column '1' is 80.0% or more likely to be an ID column", "IDColumnsDataCheck")])

    # test np.array
    assert id_cols_check.validate(np.array([[0, 1], [1, 2], [2, 3], [3, 4], [4, 5]])) == DataCheckResults(warnings=[DataCheckWarning("Column '0' is 80.0% or more likely to be an ID column", "IDColumnsDataCheck"),
                                                                                                                    DataCheckWarning("Column '1' is 80.0% or more likely to be an ID column", "IDColumnsDataCheck")])
