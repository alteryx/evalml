...
def test_label_encoder_no_y(X_y_binary):
    X, y = X_y_binary
    encoder = LabelEncoder()
    assert encoder.fit_transform(None, None) == (None, None)

    encoder = LabelEncoder()
    encoder.fit(None, None)
...
def test_target_imputer_with_X():
    ...
    encoder = LabelEncoder(impute_strategy='median')
...
def test_target_imputer_mean():
    ...
    encoder = LabelEncoder(impute_strategy='mean')
...
def test_target_imputer_constant(fill_value, y, y_expected):
    imputer = LabelEncoder(impute_strategy='constant', fill_value=fill_value)
    _, y_t = imputer.fit_transform(None, y)
    assert_series_equal(y_expected, y_t.to_series(), check_dtype=False)
...
def test_target_imputer_most_frequent():
    ...
    imputer = LabelEncoder(impute_strategy='most_frequent')
...
def test_target_imputer_woodwork_custom_overrides_returned_by_components(y_pd, has_nan, impute_strategy):
    ...
    for logical_type in override_types:
        ...
        imputer = LabelEncoder(impute_strategy=impute_strategy_to_use)
        imputer.fit(None, y)
        _, y_t = imputer.transform(None, y)
