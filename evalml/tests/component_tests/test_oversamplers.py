...
@pytest.mark.parametrize("sampling_ratio_dict,expected_dict_values", [({0: 0.5, 1: 1}, {0: 425, 1: 850}),
...
def test_oversampler_sampling_dict(oversampler, sampling_ratio_dict, expected_dict_values):
    X = np.array([[i for i in range(1000)],
                  [i % 7 for i in range(1000)],
                  [0.3 * (i % 3) for i in range(1000)]]).T
    X_ww = infer_feature_types(X, feature_types={0: 'Categorical', 1: 'Categorical'})
