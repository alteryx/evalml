...
def test_per_column_imputer_woodwork_custom_overrides_returned_by_components(logical_type, X_df, has_nan):
    y = pd.Series([1, 2, 1])
    if has_nan:
    ...
    for l in types_to_test:
        ...
        strategies = {'0': {"impute_strategy": "most_frequent"}}
        imputer = PerColumnImputer(impute_strategies=strategies)
        imputer.fit(X, y)
