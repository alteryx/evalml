...
def test_describe_component():
    ...
    try:
        smote = SMOTESampler()
        assert smote.describe(return_dict=True) == {'name': 'SMOTE Oversampler', 'parameters': {'sampling_strategy': 'auto', 'k_neighbors': 5, 'n_jobs': -1}}
        smote = SMOTENCSampler()
        assert smote.describe(return_dict=True) == {'name': 'SMOTENC Oversampler', 'parameters': {'categorical_features': [], 'sampling_strategy': 'auto', 'k_neighbors': 5, 'n_jobs': -1}}
        smote = SMOTENSampler()
        assert smote.describe(return_dict=True) == {'name': 'SMOTEN Oversampler', 'parameters': {'sampling_strategy': 'auto', 'k_neighbors': 5, 'n_jobs': -1}}
    except ImportError:
        pass
    # testing estimators
    ...
    for component_class in _all_transformers():
        ...
        for X, y, X_cols_expected in datatype_combos:
            ...
            if component_class.name == "SMOTENC Oversampler":
                component = component_class(categorical_features=[0])
