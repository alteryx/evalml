...
def _get_components_with_nullable_type_incompatibilities():
    # --> put in testing utils file
    all_native_components = all_components()

    return [
        comp
        for comp in all_native_components
...
@pytest.mark.parametrize(
    ...
    _get_components_with_nullable_type_incompatibilities(),
)
def test_components_with_nullable_type_incompatibilities(
    split_nullable_logical_types_by_compatibility,
    nullable_type_test_data,
    nullable_type_target,
    incompatible_component,
    ...
    # --> set nullable ltypes based on what's incompatible
    # ---> has to work for binary and multiclass and regression

    int_incompatibilities = incompatible_component._integer_nullable_incompatibilities
    bool_incompatibilities = incompatible_component._integer_nullable_incompatibilities

    (
        compatible_X_ltypes,
        incompatible_X_ltypes,
    ) = split_nullable_logical_types_by_compatibility(
        "X" in int_incompatibilities,
        "X" in bool_incompatibilities,
    )
    (
        compatible_y_ltypes,
        incompatible_y_ltypes,
    ) = split_nullable_logical_types_by_compatibility(
        "y" in int_incompatibilities,
        "y" in bool_incompatibilities,
    )

    for y_ltype in incompatible_y_ltypes:
        # should test all of the nullable types individually - with only one of each?
        y = nullable_type_target(ltype=y_ltype, has_nans=False)
        X = nullable_type_test_data(has_nans=False)
        # --> need to exclude ltypes that components can't handle but include nullable types
        X = X.ww.select(["numeric", "Boolean", "BooleanNullable"])

        # --> do we want to pass params in like this?  **incompatible_component.parameters
        comp = incompatible_component()

        with pytest.raises(Exception):
            comp.fit(X, y)
            if isinstance(comp, Transformer):
                comp.transform(X, y)
            elif isinstance(comp, Estimator):
                comp.predict(X)

        X_d, y_d = comp._handle_nullable_types(X, y)
        comp.fit(X_d, y_d)
        # --> need to run transform if transformer otherwise predict and predic_proba if available
        if isinstance(comp, Transformer):
            X_t, y_t = comp.transform(X_d, y_d)
            # --> the any any is confusing
            assert not X_t.isnull().any().any()
            assert not y_t.isnull().values.any()
            # --> add good assertion
        elif isinstance(comp, Estimator):
            y_pred = comp.predict(X_d)
            assert not y_pred.isnull().values.any()
            # --> figure out best way to call these only for estimators that have that
            # if is_regression(comp.supported_problem_types):
            #     y_pred_proba = comp.predict_proba(X_d)
            # # --> need a predict proba in sample? Or predict in sample for time series?
