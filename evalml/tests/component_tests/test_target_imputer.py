...
def test_target_imputer_median():
    X = pd.DataFrame([[np.nan, 0, 1, np.nan],
                      ...
                      [10, 2, 5, np.nan],
                      [6, 2, 7, 0]])
    transformer = TargetImputer(impute_strategy='median')
...
def test_target_imputer_mean():
    ...
    transformer = TargetImputer(impute_strategy='mean')
...
def test_target_imputer_constant():
    ...
    transformer = TargetImputer(impute_strategy='constant', fill_value=3)
...
def test_target_imputer_most_frequent():
    ...
    transformer = TargetImputer(impute_strategy='most_frequent')
...
def test_target_imputer_col_with_non_numeric():
    ...
    transformer = TargetImputer(impute_strategy='constant', fill_value=2)
...
@pytest.mark.parametrize("data_type", ['pd', 'ww'])
def test_target_imputer_multitype_with_one_bool(data_type, make_data_type):
...
def test_target_imputer_fit_transform_drop_all_nan_columns():
...
def test_target_imputer_transform_drop_all_nan_columns():
    X = pd.DataFrame({"all_nan": [np.nan, np.nan, np.nan],
                      "some_nan": [np.nan, 1, 0],
                      "another_col": [0, 1, 2]})
    transformer = TargetImputer(impute_strategy='most_frequent')
...
def test_target_imputer_transform_drop_all_nan_columns_empty():
...
def test_target_imputer_with_none():
...
def test_target_imputer_woodwork_custom_overrides_returned_by_components(X_df, has_nan, impute_strategy):
    y = pd.Series([1, 2, 1])
    if has_nan:
