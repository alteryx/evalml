...
def test_drop_outliers_transformer_init_with_duplicate_indices():
    with pytest.raises(ValueError, match="All input indices must be unique."):
        DropOutliersTransformer(indices_to_drop=[0, 0])


def test_drop_outliers_transformer_fit_transform():
    X = pd.DataFrame({"a column": [1, 2, 3], "another col": [4, 5, 6]})
    X_expected = X.copy()

    drop_rows_transformer_none = DropOutliersTransformer()
    ...
    drop_rows_transformer = DropOutliersTransformer(indices_to_drop=indices_to_drop)
...
def test_drop_outliers_transformer_fit_transform_with_empty_indices_to_drop():
    X = pd.DataFrame({"a column": [1, 2, 3], "another col": [4, 5, 6]})
    y = pd.Series([1, 0, 1])

    drop_rows_transformer = DropOutliersTransformer(indices_to_drop=[])
    fit_transformed = drop_rows_transformer.fit_transform(X)
...
def test_drop_outliers_transformer_index_not_in_input():
    X = pd.DataFrame({"numerical col": [1, 2]})
    y = pd.Series([0, 1], index=["a", "b"])
    drop_rows_transformer = DropOutliersTransformer(indices_to_drop=[100, 1])
    with pytest.raises(ValueError, match="do not exist in input features"):
        drop_rows_transformer.fit(X)

    drop_rows_transformer = DropOutliersTransformer(indices_to_drop=[0])
    with pytest.raises(ValueError, match="do not exist in input target"):
        drop_rows_transformer.fit(X, y)


def test_drop_outliers_transformer_nonnumeric_index():
