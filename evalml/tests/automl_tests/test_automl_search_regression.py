...
def test_random_state(X_y_regression):
    X, y = X_y_regression
    automl = AutoMLSearch(X, y, problem_type='regression', objective="R2", max_iterations=5, random_state=0,
                          n_jobs=1)
    automl.search()

    automl_1 = AutoMLSearch(X, y, problem_type='regression', objective="R2", max_iterations=5, random_state=0,
                            n_jobs=1)
    automl_1.search()
...
def test_callback(X_y_regression):
    ...
    max_iterations = 3
    automl = AutoMLSearch(X, y, problem_type='regression', objective="R2", max_iterations=max_iterations,
...
def test_plot_disabled_missing_dependency(X_y_regression, has_minimal_dependencies):
    ...
    automl = AutoMLSearch(X, y, problem_type='regression', max_iterations=3)
...
def test_plot_iterations_max_iterations(X_y_regression):
    ...
    automl = AutoMLSearch(X, y, problem_type='regression', max_iterations=3, n_jobs=1)
    automl.search()
...
def test_plot_iterations_max_time(X_y_regression):
    ...
    automl = AutoMLSearch(X, y, problem_type='regression', max_time=10, random_state=1, n_jobs=1)
...
def test_log_metrics_only_passed_directly():
    ...
    ar = AutoMLSearch(X, y, problem_type='regression', additional_objectives=[RootMeanSquaredLogError(), MeanSquaredLogError()])
...
def test_automl_allowed_pipelines_no_allowed_pipelines(X_y_regression):
    X, y = X_y_regression
    automl = AutoMLSearch(X, y, problem_type='regression', allowed_pipelines=None, allowed_model_families=[])
...
def test_automl_allowed_pipelines_specified_allowed_pipelines(mock_fit, mock_score, dummy_regression_pipeline_class, X_y_regression):
    X, y = X_y_regression
    automl = AutoMLSearch(X, y, problem_type='regression', allowed_pipelines=[dummy_regression_pipeline_class], allowed_model_families=None)
...
def test_automl_allowed_pipelines_specified_allowed_model_families(mock_fit, mock_score, X_y_regression, assert_allowed_pipelines_equal_helper):
    X, y = X_y_regression
    automl = AutoMLSearch(X, y, problem_type='regression', allowed_pipelines=None, allowed_model_families=[ModelFamily.RANDOM_FOREST])
    mock_score.return_value = {automl.objective.name: 1.0}
    ...
    automl = AutoMLSearch(X, y, problem_type='regression', allowed_pipelines=None, allowed_model_families=['random_forest'])
    expected_pipelines = [make_pipeline(X, y, estimator, ProblemTypes.REGRESSION) for estimator in get_estimators(ProblemTypes.REGRESSION, model_families=[ModelFamily.RANDOM_FOREST])]
    assert automl.allowed_pipelines is None
...
def test_automl_allowed_pipelines_init_allowed_both_not_specified(mock_fit, mock_score, X_y_regression, assert_allowed_pipelines_equal_helper):
    X, y = X_y_regression
    automl = AutoMLSearch(X, y, problem_type='regression', allowed_pipelines=None, allowed_model_families=None)
    mock_score.return_value = {automl.objective.name: 1.0}
...
def test_automl_allowed_pipelines_init_allowed_both_specified(mock_fit, mock_score, dummy_regression_pipeline_class, X_y_regression, assert_allowed_pipelines_equal_helper):
    X, y = X_y_regression
    automl = AutoMLSearch(X, y, problem_type='regression', allowed_pipelines=[dummy_regression_pipeline_class], allowed_model_families=[ModelFamily.RANDOM_FOREST])
    mock_score.return_value = {automl.objective.name: 1.0}
    expected_pipelines = [dummy_regression_pipeline_class]
    assert automl.allowed_pipelines == expected_pipelines
    assert set(automl.allowed_model_families) == set([ModelFamily.RANDOM_FOREST])
...
def test_automl_allowed_pipelines_search(mock_fit, mock_score, dummy_regression_pipeline_class, X_y_regression):
    ...
    automl = AutoMLSearch(X, y, problem_type='regression', max_iterations=2, start_iteration_callback=start_iteration_callback,
