...
from evalml import AutoBinaryClassificationSearch, AutoRegressionSearch
...
def test_pipeline_limits(caplog, X_y):
    ...
    automl = AutoBinaryClassificationSearch(max_pipelines=1)
    automl.search(X, y)
    out = caplog.text
    assert "Searching up to 1 pipelines. " in out

    caplog.clear()
    automl = AutoBinaryClassificationSearch(max_time=1)
    automl.search(X, y)
    out = caplog.text
    assert "Will stop searching for new pipelines after 1 seconds" in out

    caplog.clear()
    automl = AutoBinaryClassificationSearch(max_time=1, max_pipelines=5)
    automl.search(X, y)
    out = caplog.text
...
def test_rankings(X_y, X_y_reg):
    ...
    automl = AutoBinaryClassificationSearch(allowed_model_families=model_families, max_pipelines=3)
    automl.search(X, y)
...
def test_automl_str_search(mock_fit, X_y):
    ...
    automl = AutoBinaryClassificationSearch(**search_params)
    str_rep = str(automl)
...
def test_automl_algorithm(mock_fit, mock_score, mock_algo_next_batch, X_y):
    ...
    automl = AutoBinaryClassificationSearch(max_pipelines=5)
    automl.search(X, y)
    assert automl.data_check_results is None
    mock_fit.assert_called()
    mock_score.assert_called()
...
def test_automl_allowed_pipelines_algorithm(mock_algo_init, dummy_binary_pipeline_class, X_y):
    ...
    automl = AutoBinaryClassificationSearch(allowed_pipelines=allowed_pipelines, max_pipelines=10)
    ...
    automl = AutoBinaryClassificationSearch(allowed_model_families=allowed_model_families, max_pipelines=1)
    with pytest.raises(Exception, match='mock algo init'):
        automl.search(X, y)
