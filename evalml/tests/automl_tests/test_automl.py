...
def test_pipeline_limits(
    ...
    automl = AutoMLSearch(
        X_train=X, y_train=y, problem_type=automl_type, max_iterations=1
    )
    automl.search()
    out = caplog.text
    ...
    caplog.clear()
    automl = AutoMLSearch(
        X_train=X, y_train=y, problem_type=automl_type, max_time=1, max_iterations=5
    )
    automl.search()
    out = caplog.text
    ...
    caplog.clear()
    automl = AutoMLSearch(
        X_train=X, y_train=y, problem_type=automl_type, max_time=1e-16
    )
    automl.search()
    out = caplog.text
...
def test_automl_allowed_pipelines_algorithm(
    mock_algo_init, dummy_binary_pipeline_class, X_y_binary
):
...
def test_allowed_pipelines_with_incorrect_problem_type(
...
def test_add_to_rankings_regression(
    mock_score, dummy_regression_pipeline_class, X_y_regression
):
    X, y = X_y_regression
...
@patch(
    ...
    return_value={"Log Loss Binary": 1.0},
)
@patch("evalml.pipelines.BinaryClassificationPipeline.fit")
...
def test_describe_pipeline_with_ensembling(
    mock_pipeline_fit, mock_score, return_dict, X_y_binary, caplog
):
    X, y = X_y_binary
...
def test_targets_pandas_data_types_classification(
    ...
    automl = AutoMLSearch(
        X_train=X, y_train=y, problem_type=automl_type, max_iterations=3, n_jobs=1
    )
    automl.search()
...
def test_jobs_cancelled_when_keyboard_interrupt(
    mock_fit, mock_score, mock_cancel, mock_done, mock_input, X_y_binary
):
    X, y = X_y_binary
    automl = AutoMLSearch(
        X_train=X,
...
@patch("evalml.pipelines.BinaryClassificationPipeline.score")
@patch("evalml.pipelines.BinaryClassificationPipeline.fit")
def test_error_during_train_test_split(
    mock_fit, mock_score, mock_split_data, X_y_binary
):
    X, y = X_y_binary
    mock_score.return_value = {"Log Loss Binary": 1.0}
...
@patch(
    "evalml.pipelines.BinaryClassificationPipeline.score",
    return_value={"Log Loss Binary": 1, "F1": 1},
)
@patch("evalml.pipelines.BinaryClassificationPipeline.fit")
def test_percent_better_than_baseline_scores_different_folds(
    mock_fit, mock_score, fold_scores, dummy_binary_pipeline_class, X_y_binary
):
...
def test_max_iteration_works_with_stacked_ensemble(
    mock_pipeline_fit, mock_score, max_iterations, use_ensembling, X_y_binary, caplog
):
    X, y = X_y_binary
...
def test_automl_max_iterations_less_than_ensembling_disabled(
    mock_pipeline_fit, mock_score, X_y_binary, caplog
):
...
@patch("evalml.pipelines.BinaryClassificationPipeline.fit")
def test_max_batches_plays_nice_with_other_stopping_criteria(
...
def test_iterative_algorithm_pipeline_hyperparameters_make_pipeline_other_errors(
    ...
    custom_hyperparameters = {
        "Imputer": {"numeric_impute_strategy": ["most_frequent", "mean"]}
    }
    estimators = get_estimators("multiclass", [ModelFamily.EXTRA_TREES])

    pipelines = [
        make_pipeline(X, y, estimator, "multiclass", None, custom_hyperparameters)
...
@patch("evalml.pipelines.BinaryClassificationPipeline.score")
@patch("evalml.pipelines.BinaryClassificationPipeline.fit")
def test_iterative_algorithm_pipeline_hyperparameters_make_pipeline_errors(
    ...
    invalid_pipelines = [
        make_pipeline(
            X, y, estimator, "multiclass", None, invalid_custom_hyperparameters
        )
        for estimator in estimators
    ]
    automl = AutoMLSearch(
        ...
        y_train=y,
        problem_type="multiclass",
        allowed_pipelines=invalid_pipelines,
    )
    with pytest.raises(ValueError, match="Default parameters for components"):
        automl.search()

    invalid_pipelines = [
        make_pipeline(X, y, estimator, "multiclass", None, larger_invalid)
        for estimator in estimators
    ]
    automl = AutoMLSearch(
        X_train=X,
...
@patch("evalml.pipelines.BinaryClassificationPipeline.score")
@patch("evalml.pipelines.BinaryClassificationPipeline.fit")
def test_iterative_algorithm_pipeline_hyperparameters_make_pipeline(
    mock_fit, mock_score, X_y_multi
):
    X, y = X_y_multi
    custom_hyperparameters = {"Imputer": {"numeric_impute_strategy": ["mean"]}}
    larger_custom = {
        "Imputer": {"numeric_impute_strategy": ["most_frequent", "mean"]},
        "Extra Trees Classifier": {
            "max_depth": [4, 5, 6, 7],
    ...
    automl = AutoMLSearch(
        X_train=X, y_train=y, problem_type="multiclass", allowed_pipelines=pipelines
    )
    automl.search()
    assert automl.best_pipeline.hyperparameters["Imputer"][
        "numeric_impute_strategy"
    ] == ["mean"]

    invalid_pipelines = [
        make_pipeline(X, y, estimator, "multiclass", None, larger_custom)
        for estimator in estimators
    ]
    automl = AutoMLSearch(
        X_train=X,
...
@patch(
    "evalml.pipelines.BinaryClassificationPipeline.score",
    return_value={"Log Loss Binary": 0.6},
)
@patch("evalml.pipelines.BinaryClassificationPipeline.fit")
def test_iterative_algorithm_passes_njobs_to_pipelines(
    ...
    automl = AutoMLSearch(
        ...
        allowed_pipelines=[
            BinaryClassificationPipeline(
                [MockEstimatorWithNJobs], custom_name="Pipeline 1"
            ),
            BinaryClassificationPipeline(
                [MockEstimatorWithNJobs], custom_name="Pipeline 2"
            ),
            dummy_binary_pipeline_class({}),
        ],
    )
...
def test_automl_respects_random_seed(
    mock_fit, mock_score, X_y_binary, dummy_classifier_estimator_class
):
...
@patch("evalml.pipelines.BinaryClassificationPipeline.fit")
def test_automl_error_callback(
    mock_fit, mock_score, error_type, callback, X_y_binary, caplog
):
    X, y = X_y_binary
...
def test_automl_woodwork_user_types_preserved(
    ...
    automl = AutoMLSearch(
        X_train=X, y_train=y, problem_type=problem_type, max_batches=5
    )
    automl.search()
...
def test_automl_best_pipeline(mock_optimize, X_y_binary):
    ...
    automl = AutoMLSearch(
        X_train=X, y_train=y, problem_type="binary", train_best_pipeline=False, n_jobs=1
    )
    automl.search()
...
def test_automl_pipeline_params_simple(mock_fit, mock_score, X_y_binary):
    ...
    for i, row in automl.rankings.iterrows():
        ...
        if "Logistic Regression Classifier" in row["parameters"]:
            ...
            assert (
                row["parameters"]["Logistic Regression Classifier"]["penalty"] == "none"
            )
...
def test_automl_respects_pipeline_parameters_with_duplicate_components(
    mock_score, mock_fit, X_y_binary
):
    X, y = X_y_binary
    ...
    automl = AutoMLSearch(
        ...
        y,
        problem_type="binary",
        allowed_pipelines=[pipeline_dict, pipeline_linear],
        pipeline_parameters={
    ...
    for i, row in automl.full_rankings.iterrows():
        if "Mode Baseline Binary" in row["pipeline_name"]:
            continue
        assert (
            row["parameters"]["Imputer"]["numeric_impute_strategy"] == "most_frequent"
        )
    ...
    automl = AutoMLSearch(
        ...
        y,
        problem_type="binary",
        allowed_pipelines=[pipeline_linear, pipeline_dict],
        pipeline_parameters={
            "One Hot Encoder": {"top_n": Categorical([15])},
...
def test_automl_respects_pipeline_custom_hyperparameters_with_duplicate_components(
    ...
    pipeline_dict = BinaryClassificationPipeline(
        component_graph_dict,
        custom_name="Pipeline from dict",
...
def test_automl_adds_pipeline_parameters_to_custom_pipeline_hyperparams(
    ...
    pipeline_one = BinaryClassificationPipeline(
        component_graph,
        custom_name="Pipe Line One",
        custom_hyperparameters=custom_hyperparameters,
    )
    pipeline_two = BinaryClassificationPipeline(
        ["Imputer", "Imputer", "One Hot Encoder", "Random Forest Classifier"],
        custom_name="Pipe Line Two",
        custom_hyperparameters={"One Hot Encoder": {"top_n": Categorical([12, 10])}},
    )

    pipeline_three = BinaryClassificationPipeline(
        ["Imputer", "Imputer", "One Hot Encoder", "Random Forest Classifier"],
        custom_name="Pipe Line Three",
        custom_hyperparameters={
            "Imputer": {"numeric_imputer_strategy": Categorical(["median"])}
        },
    )
    ...
    automl = AutoMLSearch(
        ...
        y,
        problem_type="binary",
        allowed_pipelines=[pipeline_one, pipeline_two, pipeline_three],
        pipeline_parameters={
            "Imputer": {"numeric_impute_strategy": Categorical(["most_frequent"])}
        },
        max_batches=4,
    ...
    for i, row in automl.full_rankings.iterrows():
        ...
        assert (
            row["parameters"]["One Hot Encoder"]["top_n"]
            in expected_top_n[row["pipeline_name"]]
        )
    assert any(
        row["parameters"]["One Hot Encoder"]["top_n"] == 12
        for _, row in automl.full_rankings.iterrows()
        if row["pipeline_name"] == "Pipe Line Two"
    )
    assert any(
        row["parameters"]["One Hot Encoder"]["top_n"] == 5
        for _, row in automl.full_rankings.iterrows()
        if row["pipeline_name"] == "Pipe Line One"
    )
...
def test_automl_check_high_variance_logs_warning(mock_fit_binary, X_y_binary, caplog):
    ...
    with patch(
        "evalml.pipelines.BinaryClassificationPipeline.score",
        return_value={"Log Loss Binary": 1},
    ):
        automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary")
        automl.search()
        out = caplog.text
    ...
    with patch(
        "evalml.pipelines.BinaryClassificationPipeline.score",
        side_effect=desired_score_values,
    ):
        automl = AutoMLSearch(
            X_train=X, y_train=y, problem_type="binary", max_iterations=2
        )
        automl.search()
        out = caplog.text
...
def test_automl_raises_error_with_duplicate_pipeline_names(X_y_binary):
    ...
    pipeline_3 = BinaryClassificationPipeline(
        component_graph=["Logistic Regression Classifier"], custom_name="My Pipeline 3"
    )
    pipeline_4 = BinaryClassificationPipeline(
        component_graph=["Random Forest Classifier"], custom_name="My Pipeline 3"
    )
    ...
    with pytest.raises(
        ...
        AutoMLSearch(
            ...
            y,
            problem_type="binary",
            allowed_pipelines=[pipeline_1, pipeline_2, pipeline_3],
        )
    ...
    with pytest.raises(
        ...
        AutoMLSearch(
            ...
            y,
            problem_type="binary",
            allowed_pipelines=[pipeline_1, pipeline_2, pipeline_3, pipeline_4],
        )
...
def test_score_batch_before_fitting_yields_error_nan_scores(
    X_y_binary, dummy_binary_pipeline_class, caplog
):
    X, y = X_y_binary
...
def test_automl_supports_float_targets_for_classification(
    ...
    automl = AutoMLSearch(
        X_train=X, y_train=y, problem_type=automl_type, random_seed=0, n_jobs=1
    )
    automl.search()
