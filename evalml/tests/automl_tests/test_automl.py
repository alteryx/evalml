...
def test_percent_better_than_baseline_in_rankings(
    ...
    if objective.name.lower() == "cost benefit matrix":
        ...
        automl = AutoMLSearch(
            ...
            objective=objective,
            additional_objectives=[],
            problem_configuration={"date_index": None, "gap": 0, "max_delay": 0, "forecast_horizon": 2},
            train_best_pipeline=False,
            n_jobs=1,
        )
    else:
...
def test_percent_better_than_baseline_computed_for_all_objectives(
    ...
    if problem_type_enum == ProblemTypes.TIME_SERIES_REGRESSION:
        parameters = {"pipeline": {"date_index": None, "gap": 6, "max_delay": 3, "forecast_horizon": 3}}
    ...
    automl = AutoMLSearch(
        ...
        max_iterations=2,
        objective="auto",
        problem_configuration={"date_index": None, "gap": 1, "max_delay": 1, "forecast_horizon": 3},
        optimize_thresholds=False,
    ...
    automl._automl_algorithm = IterativeAlgorithm(
        ...
        ensembling=False,
        text_in_ensembling=False,
        pipeline_params={"pipeline": {"date_index": None, "gap": 1, "max_delay": 1, "forecast_horizon": 2}},
        custom_hyperparameters=None,
    )
...
def test_time_series_regression_with_parameters(ts_data):
    X, y = ts_data
    X.index.name = "Date"
    problem_configuration = {"date_index": "Date", "gap": 1, "max_delay": 0, "forecast_horizon": 2}
    automl = AutoMLSearch(
        X_train=X,
        y_train=y,
...
def test_automl_validates_problem_configuration(ts_data):
    ...
    problem_config = AutoMLSearch(
        ...
        y_train=y,
        problem_type="time series regression",
        problem_configuration={"date_index": "Date", "max_delay": 2, "gap": 3, "forecast_horizon": 2},
    ).problem_configuration
    assert problem_config == {"date_index": "Date", "max_delay": 2, "gap": 3, "forecast_horizon": 2}
...
def test_timeseries_baseline_init_with_correct_gap_max_delay(
    ...
    automl = AutoMLSearch(
        ...
        y_train=y,
        problem_type="time series regression",
        problem_configuration={"date_index": None, "gap": 6, "max_delay": 3, "forecast_horizon": 7},
        max_iterations=1,
    )
    ...
    assert automl.best_pipeline.parameters == {
        "pipeline": {"date_index": None, "gap": 6, "max_delay": 3, "forecast_horizon": 7},
        'Delayed Feature Transformer': {'date_index': None,
...
def test_automl_issues_beta_warning_for_time_series(problem_type, X_y_binary):
    ...
    with warnings.catch_warnings(record=True) as warn:
        ...
        AutoMLSearch(
            ...
            y,
            problem_type=problem_type,
            problem_configuration={"date_index": None, "gap": 0, "max_delay": 2, "forecast_horizon": 9},
        )
        assert len(warn) == 1
...
def test_automl_baseline_pipeline_predictions_and_scores_time_series(problem_type):
    ...
    expected_predictions_proba = pd.DataFrame(
        {
            0: pd.Series([1.], index=[4]),
            1: pd.Series([0.], index=[4]),
        }
    )
    if problem_type == ProblemTypes.TIME_SERIES_MULTICLASS:
        ...
        expected_predictions_proba = pd.DataFrame(
            {
                0: pd.Series([0.], index=[4]),
                1: pd.Series([1.], index=[4]),
                2: pd.Series([0.], index=[4]),
            }
        )
    ...
    automl = AutoMLSearch(
        ...
        y,
        problem_type=problem_type,
        problem_configuration={"date_index": None, "gap": 0, "max_delay": 1, "forecast_horizon": 1},
    )
    baseline = automl._get_baseline_pipeline()
    X_train, y_train = X[:4], y[:4]
    X_validation, y_validation = X[4:], y[4:]
    baseline.fit(X_train, y_train)

    expected_predictions = y.shift(1)[4:]
    if problem_type != ProblemTypes.TIME_SERIES_REGRESSION:
         expected_predictions = expected_predictions.astype("int")
    preds = baseline.predict(X_validation, None, X_train, y_train)
    pd.testing.assert_series_equal(expected_predictions, preds)
    if is_classification(problem_type):
        pd.testing.assert_frame_equal(
            expected_predictions_proba, baseline.predict_proba(X_validation, X_train, y_train)
        )
    np.testing.assert_allclose(
...
def test_data_splitter_gives_pipelines_same_data(
    ...
    if automl_type == ProblemTypes.BINARY:
        ...
        problem_configuration = {"gap": 1, "max_delay": 1, "date_index": 0, "forecast_horizon": 10}
        X, y = X_y_regression
    else:
        problem_configuration = {"gap": 1, "max_delay": 1, "date_index": 0, "forecast_horizon": 10}
        X, y = X_y_binary
