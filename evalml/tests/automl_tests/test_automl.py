...
def test_pipeline_limits(mock_fit_binary, mock_score_binary,
    ...
    caplog.clear()
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type=automl_type, optimize_thresholds=False, max_time=1)
    automl.search()
    out = caplog.text
    ...
    caplog.clear()
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type=automl_type, optimize_thresholds=False, max_time=1, max_iterations=5)
    automl.search()
    out = caplog.text
    ...
    caplog.clear()
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type=automl_type, optimize_thresholds=False)
    automl.search()
    out = caplog.text
    ...
    caplog.clear()
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type=automl_type, optimize_thresholds=False, max_time=1e-16)
    automl.search()
    out = caplog.text
...
def test_objective_score_raises(mock_score, X_y_binary, caplog):
    ...
    mock_score.side_effect = Exception(msg)
    X, y = X_y_binary
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', optimize_thresholds=False, max_iterations=1, n_jobs=1)
    automl.search()
    out = caplog.text
...
def test_rankings(X_y_binary, X_y_regression):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', allowed_model_families=model_families,
                          optimize_thresholds=False, max_iterations=3, n_jobs=1)
    automl.search()
...
def test_automl_str_search(mock_fit, mock_score, mock_predict_proba, mock_encode_targets, mock_optimize_threshold, X_y_binary):
    ...
    mock_score.return_value = {automl.objective.name: 1.0, 'F1': 0.5}
...
@patch('evalml.pipelines.BinaryClassificationPipeline.score')
@patch('evalml.pipelines.BinaryClassificationPipeline.fit')
def test_automl_feature_selection(mock_fit, mock_score, mock_predict_proba, mock_encode, mock_threshold, X_y_binary):
    X, y = X_y_binary
    mock_score.return_value = {'Log Loss Binary': 1.0, 'F1': 0.5}
    mock_predict_proba.return_value = pd.DataFrame([[1.0, 0.0] for i in range(len(X))])
...
@patch('evalml.pipelines.BinaryClassificationPipeline.score')
@patch('evalml.pipelines.BinaryClassificationPipeline.fit')
def test_automl_algorithm(mock_fit, mock_score, mock_algo_next_batch, mock_predict_proba, mock_encode, mock_optimize, X_y_binary):
    X, y = X_y_binary
    mock_score.return_value = {'Log Loss Binary': 1.0, 'F1': 0.5}
    mock_predict_proba.return_value = pd.DataFrame([[1.0, 0.0] for i in range(len(X))])
    mock_optimize.return_value = 0.5
...
def test_automl_serialization(X_y_binary, tmpdir):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', optimize_thresholds=False, max_iterations=num_max_iterations, n_jobs=1)
    automl.search()
    automl.save(path)
...
@patch('evalml.pipelines.BinaryClassificationPipeline.score')
@patch('evalml.pipelines.BinaryClassificationPipeline.fit')
def test_add_to_rankings(mock_fit, mock_score, mock_predict_proba, mock_encode, mock_threshold, dummy_binary_pipeline_class, X_y_binary):
    X, y = X_y_binary
    mock_score.return_value = {'Log Loss Binary': 1.0, 'F1': 0.5}
    mock_predict_proba.return_value = pd.DataFrame([[1.0, 0.0] for i in range(len(X))])
    mock_threshold.return_value = 0.5
...
@patch('evalml.pipelines.BinaryClassificationPipeline.score')
@patch('evalml.pipelines.BinaryClassificationPipeline.fit')
def test_add_to_rankings_no_search(mock_fit, mock_score, mock_predict_proba, mock_encode, mock_threshold, dummy_binary_pipeline_class, X_y_binary):
    X, y = X_y_binary
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', max_iterations=1,
                          allowed_pipelines=[dummy_binary_pipeline_class({})])
...
def test_add_to_rankings_duplicate(mock_fit, mock_score, dummy_binary_pipeline_class, X_y_binary):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', optimize_thresholds=False, max_iterations=1, allowed_pipelines=[dummy_binary_pipeline_class({})])
    automl.search()
    best_pipeline = automl.best_pipeline
    test_pipeline = dummy_binary_pipeline_class(parameters={})
...
@patch('evalml.pipelines.BinaryClassificationPipeline.score')
@patch('evalml.pipelines.BinaryClassificationPipeline.fit')
def test_add_to_rankings_trained(mock_fit, mock_score, mock_predict_proba, mock_encode, mock_threshold, dummy_binary_pipeline_class, X_y_binary):
    X, y = X_y_binary
    mock_score.return_value = {'Log Loss Binary': 1.0, 'F1': 0.5}
    mock_predict_proba.return_value = pd.DataFrame([[1.0, 0.0] for i in range(len(X))])
    mock_threshold.return_value = 0.5
...
def test_get_pipeline_invalid(mock_fit, mock_score, X_y_binary):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', optimize_thresholds=False, max_iterations=1)
    automl.search()
...
def test_describe_pipeline_with_ensembling(mock_pipeline_fit, mock_score, return_dict, X_y_binary, caplog):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", max_batches=two_stacking_batches,
                          objective="Log Loss Binary", ensembling=True, optimize_thresholds=False, error_callback=raise_error_callback)
...
def test_targets_pandas_data_types_classification(data_type, automl_type, target_type, make_data_type):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type=automl_type, optimize_thresholds=False, max_iterations=3, n_jobs=1)
    automl.search()
...
def test_error_during_train_test_split(mock_fit, mock_score, mock_split_data, X_y_binary):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', objective='Accuracy Binary', max_iterations=2, optimize_thresholds=False, train_best_pipeline=False)
    with pytest.raises(AutoMLSearchException, match="All pipelines in the current AutoML batch produced a score of np.nan on the primary objective"):
        automl.search()
...
def test_percent_better_than_baseline_computed_for_all_objectives(mock_time_series_baseline_regression_fit,
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type=problem_type, max_iterations=2,
                          ...
                          optimize_thresholds=False, additional_objectives=additional_objectives)
...
def test_percent_better_than_baseline_scores_different_folds(mock_fit,
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", max_iterations=2,
                          allowed_pipelines=[DummyPipeline({})], objective="log loss binary", optimize_thresholds=False, additional_objectives=["f1"])
...
def test_max_iteration_works_with_stacked_ensemble(mock_pipeline_fit, mock_score, max_iterations, use_ensembling, X_y_binary, caplog):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", max_iterations=max_iterations, objective="Log Loss Binary", optimize_thresholds=False, ensembling=use_ensembling)
    automl.search()
...
def test_max_batches_works(mock_pipeline_fit, mock_score, mock_regression_fit, mock_regression_score,
    ...
    if problem_type == ProblemTypes.BINARY:
        ...
        automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", max_iterations=None,
                              max_batches=max_batches, optimize_thresholds=False, ensembling=use_ensembling)
...
def test_automl_one_allowed_pipeline_ensembling_disabled(mock_pipeline_fit, mock_score, X_y_binary, logistic_regression_binary_pipeline_class, caplog):
    ...
    X, y = X_y_binary
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", max_iterations=max_iterations, optimize_thresholds=False, allowed_model_families=[ModelFamily.RANDOM_FOREST], ensembling=True)
    automl.search()
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", max_iterations=max_iterations, optimize_thresholds=False, allowed_pipelines=[logistic_regression_binary_pipeline_class({})], ensembling=True)
    automl.search()
    pipeline_names = automl.rankings['pipeline_name']
    assert not pipeline_names.str.contains('Ensemble').any()
    assert "Ensembling is set to True, but the number of unique pipelines is one, so ensembling will not run." in caplog.text
    ...
    caplog.clear()
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", max_iterations=max_iterations, optimize_thresholds=False, allowed_model_families=[ModelFamily.LINEAR_MODEL], ensembling=True)
    automl.search()
    pipeline_names = automl.rankings['pipeline_name']
    assert pipeline_names.str.contains('Ensemble').any()
...
def test_automl_max_iterations_less_than_ensembling_disabled(mock_pipeline_fit, mock_score, X_y_binary, caplog):
    ...
    X, y = X_y_binary
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", max_iterations=max_iterations - 1, optimize_thresholds=False, allowed_model_families=[ModelFamily.LINEAR_MODEL], ensembling=True)
    automl.search()
...
def test_max_batches_output(mock_pipeline_fit, mock_score, max_batches, X_y_binary, caplog):
    X, y = X_y_binary
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", max_iterations=None, optimize_thresholds=False, max_batches=max_batches)
    automl.search()
...
def test_max_batches_plays_nice_with_other_stopping_criteria(mock_fit, mock_score, X_y_binary):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", objective="Log Loss Binary", optimize_thresholds=False)
    automl.search()
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", objective="Log Loss Binary", max_batches=10,
                          optimize_thresholds=False, max_iterations=6)
    automl.search()
    assert len(automl.results["pipeline_results"]) == 6

    # Don't change max_iterations when only max_iterations is set
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", max_iterations=4, optimize_thresholds=False)
    automl.search()
...
def test_data_splitter_binary(mock_fit, mock_score, X_y_binary):
    ...
    y[2] = 1
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", optimize_thresholds=False, n_jobs=1)
    automl.search()
...
def test_iterative_algorithm_passes_njobs_to_pipelines(mock_fit, mock_score, dummy_binary_pipeline_class,
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', n_jobs=3, max_batches=2, optimize_thresholds=False,
...
def test_automl_ensembling_false(mock_fit, mock_score, X_y_binary):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', max_time='60 seconds', max_batches=20, optimize_thresholds=False, ensembling=False)
    automl.search()
...
def test_search_with_text(mock_fit, mock_score):
    ...
    y = [0, 1, 1, 0, 1, 0]
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', optimize_thresholds=False)
    automl.search()
...
def test_search_with_text_and_ensembling(mock_iter, df_text, problem_type, pipeline_name, ensemble_name):
    ...
    _ = AutoMLSearch(X_train=X, y_train=y, problem_type=problem_type, allowed_model_families=["random_forest", "decision_tree"],
                     optimize_thresholds=False, max_batches=4, ensembling=True)
...
def test_pipelines_per_batch(mock_fit, mock_score, X_y_binary):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', max_batches=2, optimize_thresholds=False)
    automl.search()
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', max_batches=1, optimize_thresholds=False, _pipelines_per_batch=2)
    automl.search()
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', max_batches=2, optimize_thresholds=False, _pipelines_per_batch=10)
    automl.search()
...
def test_automl_respects_random_seed(mock_fit, mock_score, X_y_binary, dummy_classifier_estimator_class):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", allowed_pipelines=pipelines,
                          optimize_thresholds=False, random_seed=42, max_iterations=10)
    automl.search()
...
def test_automl_error_callback(mock_fit, mock_score, error_type, callback, X_y_binary, caplog):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", error_callback=callback, train_best_pipeline=False, optimize_thresholds=False, n_jobs=1)
...
def test_automl_data_splitter_consistent(mock_binary_score, mock_binary_fit, mock_multi_score, mock_multi_fit,
    ...
    for seed in random_seed:
        a = AutoMLSearch(X_train=X, y_train=y, problem_type=problem_type, random_seed=seed, optimize_thresholds=False, max_iterations=1)
        a.search()
...
def test_automl_rerun(mock_fit, mock_score, X_y_binary, caplog):
    ...
    X, y = X_y_binary
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", train_best_pipeline=False, optimize_thresholds=False, n_jobs=1)
    automl.search()
    assert msg not in caplog.text
    automl.search()
    assert msg in caplog.text
...
def test_automl_pipeline_params_simple(mock_fit, mock_score, X_y_binary):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", pipeline_parameters=params, optimize_thresholds=False, n_jobs=1)
    automl.search()
...
def test_automl_pipeline_params_multiple(mock_score, mock_fit, X_y_regression):
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='regression', pipeline_parameters=params, optimize_thresholds=False, n_jobs=1)
    automl.search()
    for i, row in automl.rankings.iterrows():
        if 'Imputer' in row['parameters']:
...
def test_automl_respects_pipeline_parameters_with_duplicate_components(mock_score, mock_fit, X_y_binary):
    ...
    automl = AutoMLSearch(X, y, problem_type="binary", allowed_pipelines=[pipeline_dict, pipeline_linear],
                          ...
                          optimize_thresholds=False, max_batches=3)
    automl.search()
...
def test_automl_respects_pipeline_custom_hyperparameters_with_duplicate_components(mock_score, mock_fit, X_y_binary):
    ...
    automl = AutoMLSearch(X, y, problem_type="binary", allowed_pipelines=[pipeline_dict, pipeline_linear],
                          optimize_thresholds=False, max_batches=5)
    automl.search()
    for i, row in automl.full_rankings.iterrows():
        if "Mode Baseline Binary" in row['pipeline_name']:
            continue
...
def test_automl_adds_pipeline_parameters_to_custom_pipeline_hyperparams(mock_score, mock_fit, X_y_binary):
    ...
    automl = AutoMLSearch(X, y, problem_type="binary", allowed_pipelines=[pipeline_one, pipeline_two, pipeline_three],
                          pipeline_parameters={"Imputer": {"numeric_impute_strategy": Categorical(["most_frequent"])}},
                          optimize_thresholds=False, max_batches=4)
    automl.search()
...
def test_automl_check_high_variance_logs_warning(mock_fit_binary, X_y_binary, caplog):
    ...
    with patch('evalml.pipelines.BinaryClassificationPipeline.score', side_effect=desired_score_values):
        automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', optimize_thresholds=False, max_iterations=2)
        automl.search()
        out = caplog.text
...
def test_train_batch_works(mock_score, pipeline_fit_side_effect, X_y_binary,
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type='binary', max_time=1, max_iterations=2,
                          train_best_pipeline=False, optimize_thresholds=False, n_jobs=1)
...
def test_automl_supports_float_targets_for_classification(mock_train, mock_binary_score, mock_multi_score, mock_regression_score,
    ...
    automl = AutoMLSearch(X_train=X, y_train=y, problem_type=automl_type, optimize_thresholds=False, random_seed=0, n_jobs=1)
    automl.search()
...
@pytest.mark.parametrize("objective,errors", [("Log Loss Binary", True),
                                              ("AUC", True),
                                              ("F1", False),
                                              ("Accuracy Binary", False)])
@patch("evalml.objectives.binary_classification_objective.BinaryClassificationObjective.optimize_threshold", return_value=0.65)
def test_automl_thresholding_objective(mock_optimize, objective, errors, X_y_binary):
    X, y = X_y_binary
    if errors:
        with pytest.raises(ValueError, match="Thresholding objective must be thresholdable"):
            AutoMLSearch(X_train=X, y_train=y, problem_type="binary", thresholding_objective=objective)
    else:
        automl = AutoMLSearch(X_train=X, y_train=y, problem_type="binary", thresholding_objective=objective, max_iterations=2)
        automl.search()
