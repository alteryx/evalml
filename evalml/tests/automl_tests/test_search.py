...
def test_search(mock_automl_search, mock_data_checks_validate, X_y_binary):
    ...
    pd.testing.assert_frame_equal(mock_data_checks_validate.call_args[0][0], infer_feature_types(X))
    pd.testing.assert_series_equal(mock_data_checks_validate.call_args[1]['y'], infer_feature_types(y))
