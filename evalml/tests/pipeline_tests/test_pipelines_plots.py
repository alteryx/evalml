...
def test_returns_digraph_object():
    clf = PipelineBase('precision', component_list=['Simple Imputer', 'One Hot Encoder', 'Standard Scaler', 'Logistic Regression Classifier'], n_jobs=2, random_state=0)
    plot = clf.plot()
...
def test_saving_png_file(tmpdir):
    ...
    pipeline = PipelineBase('precision', component_list=['Simple Imputer', 'One Hot Encoder', 'Standard Scaler', 'Logistic Regression Classifier'], n_jobs=2, random_state=0)
