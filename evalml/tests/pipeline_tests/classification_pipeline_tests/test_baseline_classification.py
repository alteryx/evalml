...
def test_baseline_binary_random(X_y_binary):
    ...
    clf.fit(X, y)
    expected_predicted = pd.Series(get_random_state(0).choice(np.unique(y), len(X)), dtype="Int64")
    assert_series_equal(expected_predicted, clf.predict(X).to_series())

    predicted_proba = clf.predict_proba(X)
    assert predicted_proba.shape == (len(X), 2)
    expected_predicted_proba = pd.DataFrame(np.array([[0.5 for i in range(len(values))]] * len(X)))
    assert_frame_equal(expected_predicted_proba, predicted_proba.to_dataframe())
...
def test_baseline_binary_random_weighted(X_y_binary):
    ...
    expected_predicted = pd.Series(get_random_state(0).choice(np.unique(y), len(X), p=percent_freq), dtype="Int64")
    assert_series_equal(expected_predicted, clf.predict(X).to_series())

    expected_predicted_proba = pd.DataFrame(np.array([[percent_freq[i] for i in range(len(values))]] * len(X)))
    predicted_proba = clf.predict_proba(X)
    assert predicted_proba.shape == (len(X), 2)
    assert_frame_equal(expected_predicted_proba, predicted_proba.to_dataframe())
...
def test_baseline_binary_mode():
    ...
    expected_predicted = pd.Series(np.array([10] * len(X)), dtype="Int64")
    assert_series_equal(expected_predicted, clf.predict(X).to_series())
...
def test_baseline_multi_random(X_y_multi):
    ...
    expected_predicted_proba = pd.DataFrame(np.array([[1. / 3 for i in range(len(values))]] * len(X)))
    assert_frame_equal(expected_predicted_proba, predicted_proba.to_dataframe())
    np.testing.assert_allclose(clf.feature_importance.iloc[:, 1], np.array([0.0] * X.shape[1]))
...
def test_baseline_multi_mode():
    ...
    clf = BaselineMulticlassPipeline(parameters=parameters)
    clf.fit(X, y)
    expected_predicted = pd.Series(np.array([11] * len(X)), dtype="Int64")
    assert_series_equal(expected_predicted, clf.predict(X).to_series())
