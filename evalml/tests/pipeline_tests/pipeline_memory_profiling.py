...
def test_make_pipeline_all_nan_no_categoricals(input_type, problem_type):
    ...
    for input_type, problem_type in itertools.product(['pd', 'ww'],
...
def test_make_pipeline_numpy_input(problem_type):
    for problem_type in [ProblemTypes.REGRESSION, ProblemTypes.BINARY, ProblemTypes.MULTICLASS]:
...
def logistic_regression_binary_pipeline_class_():
...
def test_make_pipeline_from_components():
    X, y = X_y_binary_()
    logistic_regression_binary_pipeline_class = logistic_regression_binary_pipeline_class_()
...
def test_serialization():
    X, y = X_y_binary_()
    logistic_regression_binary_pipeline_class = logistic_regression_binary_pipeline_class_()
    tmpdir = "/home/Users/freddy.boulton/"
...
def pickled_pipeline_path():
    X, y = X_y_binary_()
    logistic_regression_binary_pipeline_class = logistic_regression_binary_pipeline_class_()
    path = "/Users/freddy.boulton/fixture_path.pkl"
    pipeline = logistic_regression_binary_pipeline_class(parameters={"Logistic Regression Classifier": {"n_jobs": 1}})
    pipeline.fit(X, y)
    pipeline.save(path)
    return path
...
def test_load_pickled_pipeline_with_custom_objective():
    X, y = X_y_binary_()
    logistic_regression_binary_pipeline_class = logistic_regression_binary_pipeline_class_()
    path = pickled_pipeline_path()
    objective = Precision()
    pipeline = logistic_regression_binary_pipeline_class(parameters={"Logistic Regression Classifier": {"n_jobs": 1}})
    pipeline.fit(X, y)
    assert PipelineBase.load(path).score(X, y, [objective]) == pipeline.score(X, y, [objective])
...
@profile
def test_multiple_feature_selectors():
    X, y = X_y_binary_()
...
if __name__ == "__main__":
    ...
    test_allowed_model_families()
    test_all_estimators()
    test_get_estimators()
    test_load_pickled_pipeline_with_custom_objective()