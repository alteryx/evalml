...
.PHONY: git-test
git-test:
	pytest evalml/ -n 8 --doctest-modules --cov=evalml --cov-config=../.coveragerc --junitxml=test-reports/junit.xml --doctest-continue-on-failure

.PHONY: git-test-minimal-deps
git-test-minimal-deps:
	pytest evalml/ -n 8 --doctest-modules --cov=evalml --cov-config=../.coveragerc --junitxml=test-reports/junit.xml --doctest-continue-on-failure -v --has-minimal-dependencies
