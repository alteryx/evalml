...
.PHONY: git-test
git-test:
	pytest evalml/ -v -n 2 --doctest-modules --cov=evalml --junitxml=test-reports/junit.xml --doctest-continue-on-failure
...
git-test-minimal-deps:
	pytest evalml/ -v -n 2 --doctest-modules --cov=evalml --junitxml=test-reports/junit.xml --doctest-continue-on-failure --has-minimal-dependencies
